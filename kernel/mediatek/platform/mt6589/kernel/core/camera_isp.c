...
static MINT32 ISP_DumpReg(void)
{
    ...
    //CDP
    LOG_DBG("0x%08X %08X ", ISP_ADDR + 0xCC0, ISP_RD32(ISP_ADDR + 0xCC0));
    LOG_DBG("0x%08X %08X ", ISP_ADDR + 0xD40, ISP_RD32(ISP_ADDR + 0xD40));
    //SENINF1_INT_STA
    ...
    LOG_DBG("0x%08X %08X ", ISP_ADDR + 0x150, ISP_RD32(ISP_ADDR + 0x150));
    //
...
#define SMI_LARB_NUMBER 5
        ...
        ISP_WR32(SMI_LARB0+0x14,0x400000);
        ISP_WR32(SMI_LARB1+0x14,0x400000);
        ISP_WR32(SMI_LARB2+0x14,0x400000);
        ISP_WR32(SMI_LARB3+0x14,0x400000);
        ISP_WR32(SMI_LARB4+0x14,0x400000);
        ...
        ISP_WR32(0xF0202000+0x214,0x9a7);

        LOG_DBG("LARB in reset, (0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)", \
    ...
    if(copy_from_user(&rt_buf_ctrl, (void*)Param, sizeof(ISP_BUFFER_CTRL_STRUCT)) == 0)
    {
        ...
        switch(rt_buf_ctrl.ctrl) {
            ...
            case ISP_RT_BUF_CTRL_DEQUE:
                ...
                if ( (reg_val & 0x01) || (reg_val2 & 0x01) ) {
                    ...
                    for (i=0;i<deque_buf.count;i++) {
                        ...
                        //
                        //if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
                            LOG_DBG("[rtbc][DEQUE]:TStamp(%d.%06d),i(%d),VA(0x%x),PA(0x%x),O(0x%x),2O(0x%x)", \
                        ...
                        if ( (0 != prv_tstamp) && (prv_tstamp >= tstamp) ) {
                            LOG_ERR("timestamp rollback,prv(%d),cur(%d)",prv_tstamp,tstamp);
                        }
                        prv_tstamp = tstamp;
                    }
            ...
            case ISP_RT_BUF_CTRL_IS_RDY:
                ...
                //
                //spin_unlock_irq(&(IspInfo.SpinLockRTBC));
                //
                break;
...
static MINT32 ISP_SOF_Buf_Get(unsigned long long sec,unsigned long usec)
{
    ...
    if ( 0 == pstRTBuf->dropCnt ) {
        ...
        //
        if(TempAddr == ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR))
        {
            LOG_ERR("TempAddr:Last(0x%08X) == Cur(0x%08X)",TempAddr,ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR));
            //ISP_DumpReg();
        }
        TempAddr = ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR);
        //
        if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
            LOG_DBG("[rtbc]TStamp(%d.%06d),curr(%d),imgo(0x%x),img2o(0x%x),fbc_o(0x%08x),fbc_2o(0x%08x),cq0c(0x%x)", \
...
//    LOG_DBG("- E.");
    ...
    IrqStatus[ISP_IRQ_TYPE_INTX]    = 0;//(ISP_RD32(ISP_REG_ADDR_INT_STATUSX)   & (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTX]|IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTX]));
    IrqStatus[ISP_IRQ_TYPE_DMAX]    = 0;//(ISP_RD32(ISP_REG_ADDR_DMA_INTX)      & (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAX]|IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAX]));

    if ( IrqStatus[ISP_IRQ_TYPE_INT]&ISP_IRQ_INT_STATUS_TG1_ERR_ST ||
         IrqStatus[ISP_IRQ_TYPE_INT]&ISP_IRQ_INT_STATUS_TG2_ERR_ST ) {
        LOG_ERR("Error IRQ, Status(0x%08X), Pass1_done may loss",IrqStatus[ISP_IRQ_TYPE_INT]);
    }

    //
    spin_lock(&(IspInfo.SpinLock));
    for(i=0; i<ISP_IRQ_TYPE_AMOUNT; i++)
    {
        if(IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i])
        {
...
static MINT32 ISP_probe(struct platform_device* pDev)
{
    ...
    //
    spin_lock_init(&(IspInfo.SpinLock));
    ...
    void*   pData)
{
    ...
    {
        p += sprintf(p,"+0x%08x 0x%08x", ISP_ADDR + i, ISP_RD32(ISP_ADDR + i));
    }
    p += sprintf(p,"====== dma ====");
