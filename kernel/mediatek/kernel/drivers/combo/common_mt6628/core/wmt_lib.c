...
#if defined(MT6628)
static UINT32 gPsIdleTime = STP_PSM_IDLE_TIME_SLEEP;//STP_PSM_SDIO_IDLE_TIME_SLEEP;
#else
...
wmt_lib_is_therm_ctrl_support (VOID)
{
    ...
    if (MTK_WCN_BOOL_FALSE == mtk_wcn_stp_is_ready()) {
        WMT_ERR_FUNC("STP not ready, can't do thermal ctrl\n");
        return MTK_WCN_BOOL_FALSE;
    }
