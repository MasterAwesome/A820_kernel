...
//struct stp_dbg_sys *g_stp_dbg = NULL;
...
int stp_gdb_notify_btm_dmp_wq(MTKSTP_DBG_T *stp_dbg, char *buf, int len){
    int retval = 0;
    STP_DBG_HDR_T *pHdr = NULL;
...
//#ifndef CONFIG_LOG_STP_INTERNAL
    ...
    pHdr = (STP_DBG_HDR_T *)buf;

    if(pHdr->dbg_type == STP_DBG_FW_DMP)
    {
        //printk("%s:enter coredump function.....\n",__func__);
        if(stp_dbg->btm != NULL){
