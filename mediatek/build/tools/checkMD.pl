...
my ($modem_path, $MTK_PLATFORM, $mtk_modem_support, $mtk_modem_2nd_support) = @ARGV;
...
#######################################
if ($ENV{MTK_ENABLE_MD1} eq "yes") {
	($MD_IMG_DEBUG, $MD_IMG_MODE, $MD_IMG_PLATFORM, $MD_IMG_PROJECT_ID, $MD_IMG_SERIAL_NO) = &Parse_MD($MD_IMG);
}
if ($ENV{MTK_ENABLE_MD2} eq "yes") {
	($MD_IMG_DEBUG_2ND, $MD_IMG_MODE_2ND, $MD_IMG_PLATFORM_2ND, $MD_IMG_PROJECT_ID_2ND, $MD_IMG_SERIAL_NO_2ND) = &Parse_MD($MD_IMG_2ND);
}
