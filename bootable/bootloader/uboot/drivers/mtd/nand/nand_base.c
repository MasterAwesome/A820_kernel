...
/* Jauping add here 20090729 { */
unsigned int nand_flash_man_code;
unsigned int nand_flash_dev_id;
/* Jauping add here 20090729 } */
...
static int nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
...
#if (CONFIG_MTD_NAND_MTK)		
    int i, tmp = mtd->erasesize / mtd->writesize;	
    int iaCheckPageID[4];
    char* p = chip->oob_poi;
#else	
    u16 bad;
    ...
    iaCheckPageID[0] = 0;	
    iaCheckPageID[1] = 1;	
    iaCheckPageID[2] = tmp - 3;	
    iaCheckPageID[3] = tmp - 1;	
    for (i = 0; i < 4; i++) {		
        chip->cmdfunc(mtd, NAND_CMD_READOOB, 0, page + iaCheckPageID[i]);       
        chip->read_buf(mtd, chip->oob_poi, mtd->oobsize);       
        if (p[0] != 0xFF || p[8] != 0xFF || p[16] != 0xFF || p[24] != 0xFF) {           
            res = 1;            
            break;      
        }   
    }
						  ...
						  int busw, int *maf_id)
{
	...
	*maf_id = chip->read_byte(mtd);
    ...
    nand_flash_man_code = *maf_id;
	nand_flash_dev_id   = dev_id;
